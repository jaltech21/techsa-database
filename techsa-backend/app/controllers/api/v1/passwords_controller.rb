...
module Api
  module V1
    class PasswordsController < ApplicationController
      ...
      # Accepts { member: { email } } — sends reset link if email is found.
      ...
      def create
        ...
        if member
          ...
          deliver_reset_email(member, reset_url)
        end
      ...
      # Accepts { member: { reset_password_token, password, password_confirmation } }
      def update
      ...
      def deliver_reset_email(member, reset_url)
        ...
        Rails.logger.error("Resend password reset email error: #{e.message}")
      end
      ...
      def reset_email_html(name, reset_url)
        <<~HTML
          <div style="font-family:sans-serif;max-width:500px;margin:0 auto;padding:36px 32px;background:#f9fafb;border-radius:12px;">
            ...
            <p style="color:#6b7280;font-size:13px;line-height:1.6;">This link expires in <strong>6 hours</strong>. If you didn't request a password reset, you can safely ignore this email — your password will remain unchanged.</p>
